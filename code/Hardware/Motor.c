...
void MotorB_SetSpeed(int8_t Speed)
{
	if (Speed >= 0)							//如果设置正转的速度值
	{
		GPIO_ResetBits(GPIOA, GPIO_Pin_0);	//PA0置高电平
		GPIO_SetBits(GPIOA, GPIO_Pin_3);	//PA3置低电平，设置方向为正转
		PWM_SetCompare3(Speed);				//PWM设置为速度值
	}
	else									//否则，即设置反转的速度值
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_0);	//PA0置低电平
		GPIO_ResetBits(GPIOA, GPIO_Pin_3);	//PA3置高电平，设置方向为反转
